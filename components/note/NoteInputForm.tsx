...
import { InputData, NoteCreate, noteInputCreateSchema, NoteInputData } from "@/types";
...
type NoteInputFormProps = {
  inputId: number;
  setNotes: React.Dispatch<React.SetStateAction<NoteInputData[]>>;
...
export default function NoteInputForm({ inputId, setNotes, setInput, changeModalVisible}: NoteInputFormProps) {
  ...
  const handleSubmit = async () => {
    try {
      if (
        [note.date, note.description, note.quantity, note.type].includes("")
      ) {
      ...
      const response = noteInputCreateSchema.safeParse(data);
      if (response.success) {
